...
-- Nombre del estudiante: Leonardo Dolande
-- Carnet: 19-10181
-------------------------------------------------------------------------------
...
esPalindromo [_]= True
esPalindromo xs 
    |head xs == last xs = esPalindromo (init (tail xs))
    |otherwise = False
...
productoParesRec (x:xs) 
    |  mod x 2 == 0 = x * productoParesRec xs
...
parsearCondicional [] = []
parsearCondicional (x:xs) =
  case readMaybe x :: Maybe Int of
    Just n  -> Right n : parsearCondicional xs
    Nothing -> Left (map toUpper x) : parsearCondicional xs
